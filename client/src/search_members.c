...
void create_row(gchar* search_members, GObject *listbox, char chatName[32])
{
    GtkListBoxRow  *row = gtk_list_box_get_selected_row(GTK_LIST_BOX(listbox));//создание строки 
    GtkWidget *username = gtk_label_new(search_members);//создание лейбла с именем
    ...
    while(1) {
        if (update_responce_int == 1) {
            ...
            while (sqlite3_step(stmt) != SQLITE_DONE) {
                ...
                addY(name);
                create_row(name, listbox, chatName1);
            }
...
int search_member(gchar* search_members, GObject *listbox) {
    while(1) {
...
void mx_search_members(GtkButton *btn, GObject *listbox)
{
    ...
    search_member(search_members, listbox);
...
void mx_add_room()
{
    ...
    printf("CHOISE: %d, %s\n", chosen_chat_id, ChatsArray[chosen_chat_id].username);
